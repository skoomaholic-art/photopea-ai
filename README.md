...
- ТОП10 — мастер-холст 800 × 1400 с отдельными слоями background, logo, locked darkening и locked ranking number.
...
ТОП10 встроен в существующий Poster Editor как отдельное рабочее пространство с физическим master canvas 800 × 1400. Preview масштабируется CSS, backing canvas не уменьшается.

Слои: TOP_NUMBER (locked), LOGO, BOTTOM_DARKENING (locked), BACKGROUND_IMAGE. Затемнение допускает изменение цвета, интенсивности и начала градиента без изменения геометрии. Номер 1–10 автоматически центрируется и имеет тёмную заливку и настраиваемую gradient-обводку.

BACKGROUND_IMAGE использует общий недеструктивный ImageFilters / FilterStudio: Brightness, Contrast, Exposure, Saturation, Temperature, Tint, Highlights, Shadows, Whites, Blacks, Hue, Sharpen, Blur, Grain, Vignette и существующие presets. Live preview обновляет TOP10 canvas, Apply сохраняет фильтры в проекте и экспорте.

PhotopeaBridge распознаёт 800 × 1400 как TOP10 и поддерживает явный маршрут «В ТОП10». Экспорт TOP10 — PNG 800 × 1400.
